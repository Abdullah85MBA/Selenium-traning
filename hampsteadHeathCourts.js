...
async function Login(driver, credentials) {
  ...
  let UNField = await getElement(driver, `//*[@id="122:2;a"]`);
  ...
  let loginButton = await getElement(
    driver,
  ...
  await driver.wait(
    until.urlContains(
      "https://clubspark.lta.org.uk/ParliamentHillFieldsTennisCourts/Booking/BookByDate"
    ),
    10000
  );
}
...
async function Bookcourt(driver) {
  async function performActionAfterTextAppears(driver) {
    ...
    const nextSunday = getNextDay(currentDate); // Get the next Sunday
  ...
  // Find the court that is available and time after 10AM
  // get the web element class= "resource session" and data availability=true and data start tiem
  // data capacity and start date

  // Example: capacity = 4 , resourceSessionStartDate = 420
  // Loop iteration 1 : currentStartDate = 480, remainingCapacity = 3, capacityUsed =1
  // Loop iteration 2 : currentStartDate = 540, remainingCapacity = 2, capacityUsed =2
  // Loop iteration 3 : currentStartDate = 600, remainingCapacity = 1, capacityUsed =3
  function IsAvailableAfter9(capacity, resourceSessionStartDate) {
    let capacityUsed = 0; // number of hours increments from the original start date
    let remainingCapacity = capacity;
    let currentStartDate = resourceSessionStartDate;
    let hour9InMin = 540;
    while (remainingCapacity > 0) {
      if (currentStartDate > hour9InMin) {
        return true;
      }
      currentStartDate = currentStartDate + 60;
  ...
  // will be used if there is a slot available after 9 within the resource session
  function getSlotCoordinats(capacity, resourceSessionStartDate) {
    let capacityUsed = 0; // number of hours increments from the original start date
    let remainingCapacity = capacity;
    let currentStartDate = resourceSessionStartDate;
    let hour9InMin = 540;
